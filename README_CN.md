...
- 📖 [教程](docs/zh/tutorial.md) - 包含示例和最佳实践的完整指南
...
- 📖 [教程](docs/zh/tutorial.md) - 包含示例和最佳实践的完整指南
- 📚 [API 文档](docs/api/index.html) - 详细的 API 参考（运行 `make docs`）
...
💡 **提示**：查看 [教程](docs/zh/tutorial.md) 获取详细的使用指南和常见模式示例。
...
- **基于定时器**：每条消息约11.2ms（约89 msg/sec）
- **事件驱动（UVZMQ）**：每条消息约0.056ms（约17,857 msg/sec）
- **提升**：约200倍

### 吞吐量数据

使用 IPC 传输模式测试（Release 模式，`-O2` 优化）：

| 模式              | 消息大小 | 吞吐量             | 延迟      |
| ----------------- | -------- | ------------------ | --------- |
| REQ/REP（往返）   | 64B      | ~22,000 msg/sec    | ~0.045 ms |
| REQ/REP（往返）   | 1KB      | ~21,000 msg/sec    | ~0.047 ms |
| PUSH/PULL（单向） | 64B      | ~5,800,000 msg/sec | -         |
| PUSH/PULL（单向） | 1KB      | ~1,300,000 msg/sec | -         |
| PUSH/PULL（单向） | 64KB     | ~67,000 msg/sec    | -         |

### 传输模式对比

IPC（Unix 域套接字）在本地通信中比 TCP 性能更好：

- **小消息**：IPC 比 TCP 快约 22%
- **中等消息**：IPC 比 TCP 快约 25%
- **大消息**：IPC 比 TCP 快约 50%
