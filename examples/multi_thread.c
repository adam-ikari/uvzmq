...
static void *server_thread_func(void *arg)
{
    ...
    uvzmq_socket_new(data->loop, zmq_sock, server_on_recv, NULL, data, &uvzmq_sock);
    
    printf("[SERVER] Ready to receive messages\n");
    
    // Run event loop
    for (int i = 0; i < 200 && *data->received_count < data->message_count; i++) {
...
static void *client_thread_func(void *arg)
{
    ...
    for (int i = 0; i < data->message_count; i++) {
        ...
        if (rc >= 0) {
            ...
            printf("[CLIENT] Reply: %.*s\n", (int)size, (char *)zmq_msg_data(&reply));
        } else {
...
// Subscriber
static void sub_on_recv(uvzmq_socket_t *socket, zmq_msg_t *msg, void *user_data)
{
    (void)socket;
...
static void *sub_thread_func(void *arg)
{
    ...
    uvzmq_socket_t *uvzmq_sock = NULL;
    uvzmq_socket_new(data->loop, sub_sock, sub_on_recv, NULL, data->sub_received, &uvzmq_sock);
