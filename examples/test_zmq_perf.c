...
static void *server_thread(void *arg)
{
    ...
    zmq_bind(zmq_sock, "tcp://*:5902");
    
    zmq_pollitem_t items[1];
    items[0].socket = zmq_sock;
    items[0].events = ZMQ_POLLIN;
    
    g_perf_data.poll_count = 0;
    int received = 0;
    ...
    while (received < g_perf_data.msg_count) {
        ...
        if (items[0].revents & ZMQ_POLLIN) {
            ...
            while (received < g_perf_data.msg_count) {
                ...
                int recv_rc = zmq_msg_recv(&msg, zmq_sock, ZMQ_DONTWAIT);
                if (recv_rc >= 0) {
...
static void *client_thread(void *arg)
{
    ...
    zmq_connect(zmq_sock, "tcp://127.0.0.1:5902");
...
{
    printf("========================================\n");
    printf("Native ZMQ Performance Analysis\n");
    printf("========================================\n\n");
    ...
    for (int test = 0; test < 3; test++) {
        ...
        printf("=== Test %d: %d bytes x %d messages ===\n", 
               test + 1, g_perf_data.msg_size, g_perf_data.msg_count);
        ...
        printf("[SERVER] Poll calls: %d\n", g_perf_data.poll_count);
        printf("[SERVER] Avg messages per poll: %.2f\n", 
               (double)g_perf_data.msg_count / g_perf_data.poll_count);
        printf("[SERVER] Time per message: %.2f us\n", 
               (double)g_perf_data.total_time_us / g_perf_data.msg_count);
        printf("[SERVER] Time per poll: %.2f us\n", 
               (double)g_perf_data.total_time_us / g_perf_data.poll_count);
        printf("\n");
